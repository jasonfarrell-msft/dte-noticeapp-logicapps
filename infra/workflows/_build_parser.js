...
//   - Event-driven: HTTP_Request trigger (called by downloader after raw blob write)
//     Removed 5-minute recurrence polling and blob list/for-each loop.
const fs = require('fs');
...
const definition = {
      ...
      "inputs": {
        "schema": {
          ...
          "properties": {
            "rawBlobPath": {
              "type": "string",
              "description": "Full blob path within container: raw/{source}/{date}/{pipeline}/{noticeId}.html"
            }
          },
          "required": ["rawBlobPath"]
        }
      }
    }
  },
  "actions": {
    ...
    "Parse_Blob_Path": {
      "type": "Compose",
      "inputs": {
        "blobPath": "@{triggerBody()?['rawBlobPath']}",
        "blobName": "@{last(split(triggerBody()?['rawBlobPath'], '/'))}",
        "pathParts": "@split(triggerBody()?['rawBlobPath'], '/')"
      },
    ...
    "Response_Success": {
      ...
      "inputs": {
        ...
        "body": {
          "status": "success",
          "rawBlobPath": "@{triggerBody()?['rawBlobPath']}"
        }
      },
      "runAfter": { "Process_If_Not_Parsed": ["Succeeded"] },
      "kind": "Http"
    },
    "Response_Failure": {
      "type": "Response",
      ...
      "runAfter": {
        "Parse_Blob_Path": ["Failed"],
        "Extract_Path_Metadata": ["Failed"],
        "Process_If_Not_Parsed": ["Failed", "TimedOut"]
      },
      "kind": "Http"
    }
  },
