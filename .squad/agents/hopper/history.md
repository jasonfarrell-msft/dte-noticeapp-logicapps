...
- 2026-05-11: Redeploy + backfill design: seed new environments by copying `critical-notices/notices/**` (for dedup/idempotency) plus ≥10 days of `critical-notices/processed/raw/**` and `critical-notices/indices/daily/**`. Run backfill from within the VNet (storage is private). Use copy mode that does **not overwrite** existing destination blobs. Decision captured in `.squad/decisions/inbox/hopper-redeploy-backfill.md`.
